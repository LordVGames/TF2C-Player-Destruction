...
int g_DomLogicScore_Green, g_DomLogicScore_Yellow;
bool g_Logic_HasRedFirstFlagStolen, g_Logic_HasBlueFirstFlagStolen;
bool g_Logic_HasGreenFirstFlagStolen, g_Logic_HasYellowFirstFlagStolen;
...
public Action EntityIO_OnEntityInput(int entity, char input[256], int& activator, int& caller, EntityIO_VariantInfo variantInfo, int actionId)
{
	...
	//#region AddTeamPoints & SetTeamPoints
	/**
	 * More specifically:
	 * - AddRedPoints, SetRedPoints
	 * - AddBluePoints, SetBluePoints
	 * - AddGreenPoints, SetGreenPoints
	 * - AddYellowPoints, SetYellowPoints
	 */
	if (
		input[6] == 'P' ||
		input[7] == 'P' ||
		...
		else if (input[3] == 'G')
		{
			if (input[0] == 'A')
			{
				g_DomLogicScore_Green += inputValue;
			}
			else if (input[0] == 'S')
			{
				g_DomLogicScore_Green = inputValue;
			}
			FireCustomOutput("OnGreenScoreChanged", g_DomLogicEnt, activator, true, g_DomLogicScore_Green);
		}
		else if (input[3] == 'Y')
		{
			if (input[0] == 'A')
			{
				g_DomLogicScore_Yellow += inputValue;
			}
			else if (input[0] == 'S')
			{
				g_DomLogicScore_Yellow = inputValue;
			}
			FireCustomOutput("OnYellowScoreChanged", g_DomLogicEnt, activator, true, g_DomLogicScore_Yellow);
		}
	}
	//#endregion
